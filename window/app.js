...
route(/^fixes$/, async () => {
  ...
  ${findings ? `<h2>Findings tally-stick led, and what became of them <span class="mute small">(from its record, every publish)</span></h2><div class="card scroll"><table class="t findings"><tr><th>when</th><th>finding</th><th>fix proposed</th><th>outcome</th><th>what happened</th></tr>${findings.map((f) => `<tr><td class="mono">${esc(f.date)}</td><td>${f.post_id ? `<a href="#/post/${f.post_id}">${esc(f.title)}</a>` : `<a href="${esc(f.pr_url)}" rel="noopener">${esc(f.title)}</a> <span class="mute small">(pull request, no post)</span>`}</td><td class="small">${(f.prs || []).map((x) => `<a href="${esc(x.url)}" rel="noopener">PR ${x.number}</a> <span class="pill ${x.state === "merged" ? "ok" : x.state === "closed" ? "bad" : "warn"}">${esc(x.state)}</span>${x.hours_to_merge != null ? ` <span class="mute">${x.hours_to_merge}h</span>` : ""}`).join("<br>") || '<span class="mute">—</span>'}</td><td class="small nowrap"><span class="pill ${f.outcome_class || ""}">${esc(f.outcome)}</span></td><td class="small note">${esc(f.outcome_note || "") || '<span class="mute">—</span>'}</td></tr>`).join("")}</table></div>` : ""}
